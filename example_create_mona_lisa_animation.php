...
$output_directory="test_u_eriknstr/";
...
$skip_frames=40;
	...
	$start_frame=0;
	$end_frame=$p->maxframes;
	// $end_frame=14000;
	// echo $p->maxframes." mexframes\r\n";
	// exit;
	
	
	for($frame=$start_frame;$frame<=$end_frame-$skip_frames;$frame+=$skip_frames){
		...
		for($j=$jstart;$j<min(1000,$jstart+$height);$j++){
			...
			for($frame=$start_frame;$frame<=$end_frame-$skip_frames;$frame+=$skip_frames){
				...
				if($num_edits_temp>0){
					...
					$last_activity_g=$last_activity_g*0.9;
				}
				if($num_edits_temp>=5){
					$last_activity_r=150;
					$last_activity_b=150;		
					...
					$last_activity_r=$last_activity_r*0.8;
